...
const toPerson = (u = {}) => {
  ...
  return {
    id: u._id || u.id || u.email || u.name || Math.random().toString(36).slice(2),
    name: u.name || u.fullName || u.displayName || u.email || "User",
    avatar: u.avatar || u.photo || u.image || u.profilePic || u.avatar_url || u.picture || "",
  };
};
...
const colorFromName = (name = "") => {
  ...
  let sum = 0; for (let i = 0; i < name.length; i++) sum = (sum + name.charCodeAt(i)) % 997;
  ...
  return (
    <Stack direction="row" alignItems="center" gap={0.75}>
      ...
      >
        {shown.map((p, i) => {
          ...
          return (
            <Tooltip key={p.id || i} arrow placement="top" title={name}>
              <Avatar size={size} src={src || undefined} variant={src ? "soft" : "solid"} color={src ? "neutral" : color} sx={ringSx}>
                {!src && initials}
              </Avatar>
            </Tooltip>
          );
        ...
        {extra.length > 0 && (
          <Tooltip
            arrow placement="bottom" variant="soft"
            title={
              <Box sx={{ maxHeight: 260, overflowY: "auto", px: 1, py: 0.5, maxWidth: 240 }}>
                {extra.map((p, i) => (
                  <Box key={p.id || i} sx={{ mb: i !== extra.length - 1 ? 1 : 0 }}>
                    <Stack direction="row" alignItems="center" gap={1}>
                      <Avatar size="sm" src={p.avatar || undefined} variant={p.avatar ? "soft" : "solid"} color={p.avatar ? "neutral" : colorFromName(p.name)} sx={ringSx}>
          ...
          >
            <Avatar size={size} variant="soft" color="neutral" sx={{ ...ringSx, ml: "-8px", fontSize: 12, cursor: "default" }}>
              +{extra.length}
            </Avatar>
          </Tooltip>
        )}
...
const Connector = ({ durationLabel = "" }) => (
  <Box sx={{ position: "relative", minWidth: 160, mx: 1 }}>
    {durationLabel && (
      <Typography
        level="body-xs"
        sx={{ position: "absolute", top: -18, left: "50%", transform: "translateX(-50%)", color: "text.tertiary", whiteSpace: "nowrap" }}
      >
        {durationLabel}
      </Typography>
    )}
    <Box sx={{ borderTop: "2px dashed", borderColor: "neutral.outlinedBorder", height: 0, width: "100%", position: "relative", mt: 1 }}>
      <Box
        sx={{
          position: "absolute", right: -8, top: -6, width: 0, height: 0,
          borderTop: "6px solid transparent", borderBottom: "6px solid transparent",
          borderLeft: "8px solid", borderLeftColor: "neutral.outlinedBorder",
        }}
      />
    </Box>
  </Box>
...
const StatusTimeline = ({ history = [], current }) => {
  ...
  if (current?.status && (!last || (last.status || "").toLowerCase() !== (current.status || "").toLowerCase())) {
    steps.push({ status: current.status, updatedAt: current.updatedAt || new Date().toISOString() });
  }

  const nodes = steps.map((s) => ({ label: cap(s.status || "-"), color: statusColor(s.status), at: s.updatedAt ? new Date(s.updatedAt) : null }));
  ...
  return (
    <Sheet variant="soft" sx={{ p: 2, borderRadius: "md", bgcolor: "background.level1", overflowX: "auto" }}>
      <Stack direction="row" alignItems="center" gap={2} sx={{ minWidth: "max-content" }}>
        {nodes.length === 0 ? (
          <Typography level="body-sm" sx={{ color: "text.tertiary" }}>No status changes yet.</Typography>
        ) : (
          nodes.map((n, i) => (
            <Box key={`${n.label}-${i}`} sx={{ display: "flex", alignItems: "center" }}>
              <Chip variant="solid" color={n.color} sx={{ borderRadius: "8px", px: 1.25, py: 0.25, minWidth: 96, justifyContent: "center" }}>
                {n.label}
              </Chip>
        ...
        {!isClosed && nodes.length > 0 && (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Connector durationLabel={formatDuration(Date.now() - (nodes[nodes.length - 1]?.at?.getTime?.() ?? Date.now()))} />
            <Box
              sx={{
                position: "relative",
                px: 1.5, py: 0.5, bgcolor: "neutral.softBg", borderRadius: "999px",
                "&:after": {
...
export default function ViewTaskPage() {
  ...
  useEffect(() => { if (taskApi) setTask(taskApi); }, [taskApi]);
  ...
  const projects = useMemo(() => {
    if (Array.isArray(task?.project_id) && task?.project_id?.length) {
      return task.project_id.map((p) => ({ code: p.code ?? p.projectCode ?? "-", name: p.name ?? p.projectName ?? "-" }));
    }
    if (task?.project) return [{ code: task.project.code ?? "-", name: task.project.name ?? "-" }];
    if (task?.project_code || task?.project_name) return [{ code: task.project_code ?? "-", name: task.project_name ?? "-" }];
    return [];
  }, [task]);
  ...
  const handleStatusSubmit = async () => {
    ...
    try {
      ...
      const entry = { status: chosen, remarks: note, user_id: { name: user?.name || "You" }, updatedAt: new Date().toISOString() };
      setTask((prev) => ({
        ...prev,
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      if (hasFiles) {
        ...
        form.append("data", JSON.stringify({ comment: html, attachmentNames: attachments.map((a) => a.name) }));
        ...
        const updated = await updateTask({ id, body: { comment: html } }).unwrap();
        setTask(updated);
  ...
  const infoGrid = (
    <Sheet variant="soft" sx={{ p: 2, borderRadius: "md", bgcolor: "background.level1", gap: 2, display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, width: "100%" }}>
      {/* LHS */}
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        <Field label="Assign" value={task?.assigned_to?.length ? <PeopleAvatars people={toPeople(task.assigned_to)} /> : "None"} />
        <Field
          label="Status"
          value={
            <Chip size="sm" color={statusColor(task?.current_status?.status)} onClick={() => !isCompleted && setOpenStatusModal(true)} sx={{ cursor: isCompleted ? "not-allowed" : "pointer" }}>
              {task?.current_status?.status ? task.current_status.status.split(" ").map((w) => w[0]?.toUpperCase() + w.slice(1)).join(" ") : "—"}
            </Chip>
          }
        />
        <Field
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        ...
        <Field label="Created At" value={task?.createdAt ? new Date(task.createdAt).toLocaleString() : "—"} />
        <Field label="Due Date" value={task?.deadline ? new Date(task.deadline).toLocaleDateString("en-GB") : "—"} />
      </Box>
    </Sheet>
  );
  ...
  const activity = useMemo(() => {
    ...
    return [...statuses, ...comments].sort((a, b) => b.at - a.at);
  }, [task]);

  return (
    <Box sx={{ ml: { xs: 0, lg: "var(--Sidebar-width)" }, width: { xs: "100%", lg: "calc(100% - var(--Sidebar-width))" }, bgcolor: "background.body" }}>
      {/* hidden file input for modal Browse */}
      <input ref={filePickerRef} type="file" multiple onChange={(e) => { const files = Array.from(e.target.files || []); addPickedFiles(files); e.target.value = ""; }} style={{ display: "none" }} />

      {/* Row 1: Summary + Task Info */}
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2, alignItems: "start", mb: 2, "& > *": { minWidth: 0 } }}>
        <Sheet variant="outlined" sx={{ p: 2, borderRadius: "md", display: "flex", alignItems: "flex-start", justifyContent: "flex-start", mb: 0, bgcolor: "background.surface", height: "100%", minWidth: 0, overflowX: "hidden" }}>
          <Stack direction="column" gap={1.5} alignItems="flex-start" sx={{ minWidth: 0, width: "100%" }}>
            <Stack direction="row" gap={1} alignItems="center" justifyContent="flex-start" sx={{ minWidth: 0 }}>
              {task?.taskCode && <Chip size="sm" variant="soft" color="primary">{task.taskCode}</Chip>}
              <Chip variant="soft" color={statusColor(task?.current_status?.status)} onClick={() => !isCompleted && setOpenStatusModal(true)} sx={{ cursor: isCompleted ? "not-allowed" : "pointer" }}>
                {(task?.current_status?.status || "Current Status").split(" ").map((w) => w[0]?.toUpperCase() + w.slice(1)).join(" ")}
              </Chip>
            ...
            <Box sx={{ maxHeight: 72, overflowY: "auto", overflowX: "hidden", width: "100%", pr: 0.5 }}>
              <Typography level="h4" sx={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere", wordBreak: "break-word", minWidth: 0 }}>
            ...
            <Box sx={{ maxHeight: 180, overflowY: "auto", overflowX: "hidden", width: "100%", pr: 0.5 }}>
              <Typography level="body-sm" sx={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere", wordBreak: "break-word", minWidth: 0 }}>
                {task?.description || "—"}
              </Typography>
            </Box>
          </Stack>
        ...
        <Sheet variant="outlined" sx={{ p: 2, borderRadius: "md", display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "flex-start", mb: 0, bgcolor: "background.surface", height: "100%", minWidth: 0, overflowX: "hidden" }}>
          <Typography fontSize={"1rem"} fontWeight={600} mb={1}>Task Information</Typography>
          {infoGrid}
        </Sheet>
      </Box>

      {/* Row 2: Status Timeline (if project) */}
      {task?.type === "project" && (
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2, alignItems: "start", mb: 2, "& > *": { minWidth: 0 } }}>
          <Section title="Associated Project" open={openAssocProject} onToggle={() => setOpenAssocProject((v) => !v)} outlined={false}>
            <Sheet variant="soft" sx={{ p: 2, borderRadius: "md", display: "flex", flexDirection: "column", gap: 1.25 }}>
              {projects.map((p, i) => (
                <Box key={`${p.code}-${i}`} sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
                  <Avatar size="md">{p?.name?.[0]?.toUpperCase() || "P"}</Avatar>
                  <Box>
                    <Typography level="body-md" fontWeight="lg">{p?.name || "Associated Project"}</Typography>
                    <Typography level="body-sm" sx={{ color: "text.tertiary" }}>{p?.code || "—"}</Typography>
                  </Box>
                </Box>
              ))}
            </Sheet>
      ...
      >
        ...
        <CommentComposer
          ...
          onCancel={() => { setCommentText(""); setAttachments([]); }}
        ...
        {/* Activity Stream: status changes + comments */}
        <Typography level="title-sm" sx={{ mb: 1 }}>Activity Stream</Typography>
        <Box sx={{ maxHeight: 320, overflow: "auto" }}>
          {activity.length === 0 ? (
            <Typography level="body-sm" sx={{ color: "text.tertiary" }}>No activity yet.</Typography>
          ) : (
            activity.map((it, idx) => {
              ...
              if (it._type === "status") {
                return (
                  <Box key={`st-${idx}`} sx={{ mb: 1.25 }}>
                    <Chip size="sm" variant="soft" color={statusColor(it.status)} sx={{ mr: 1 }}>
                      {(it.status || "-").split(" ").map((w) => w[0]?.toUpperCase() + w.slice(1)).join(" ")}
                    </Chip>
                    <Typography level="body-xs" sx={{ color: "text.tertiary" }}>
                      by {it?.user?.name || "Unknown"} on {when}
                    </Typography>
                    {it?.remarks && <Typography level="body-sm" sx={{ mt: 0.25 }}>{it.remarks}</Typography>}
                    <Divider sx={{ mt: 1 }} />
                  </Box>
                );
              }
              return (
                <Box key={`cm-${idx}`} sx={{ mb: 1.25 }}>
                  <Chip size="sm" variant="soft" color="neutral" startDecorator={<ChatBubbleOutlineRoundedIcon />} sx={{ mr: 1 }}>
                    Comment
                  </Chip>
                  <Typography level="body-xs" sx={{ color: "text.tertiary" }}>
                    by {it?.user?.name || "User"} on {when}
                  </Typography>
                  {it?.html && (
                    <div
                      className="comment-body"
                      style={{ lineHeight: 1.66, wordBreak: "break-word", marginTop: 4 }}
                      dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(it.html) }}
                    />
                  )}
                  {Array.isArray(it.attachments) && it.attachments.length > 0 && (
                    <Stack direction="row" gap={1} flexWrap="wrap" sx={{ mt: 0.5 }}>
                      {it.attachments.map((a, j) => (
                        <Chip key={j} size="sm" variant="soft">{a.name}</Chip>
                      ))}
                    </Stack>
                  )}
                  <Divider sx={{ mt: 1 }} />
      ...
      <Modal open={openStatusModal} onClose={() => setOpenStatusModal(false)}>
        <ModalDialog variant="outlined" sx={{ maxWidth: 520 }}>
          ...
          <DialogActions>
            <Button variant="outlined" onClick={() => setOpenStatusModal(false)} sx={{ color: "#3366a3", borderColor: "#3366a3", backgroundColor: "transparent", "--Button-hoverBg": "#e0e0e0", "--Button-hoverBorderColor": "#3366a3", "&:hover": { color: "#3366a3" }, height: "8px" }}>
              Cancel
            </Button>
            <Button onClick={handleStatusSubmit} loading={isUpdating} disabled={isCompleted} sx={{ backgroundColor: "#3366a3", color: "#fff", "&:hover": { backgroundColor: "#285680" }, height: "8px" }}>
              Submit
            </Button>
          </DialogActions>
        </ModalDialog>
      ...
      <Modal open={openAttachModal} onClose={() => setOpenAttachModal(false)}>
        <ModalDialog variant="outlined" sx={{ maxWidth: 560 }}>
          ...
          <Stack gap={1.25} sx={{ mt: 1 }}>
            <Input placeholder="Document name (optional)" value={attachDocName} onChange={(e) => setAttachDocName(e.target.value)} />
            ...
            >
              <Stack direction="row" gap={1} alignItems="center" justifyContent="center">
                ...
                <Typography level="body-sm">Drag & drop files here, or <strong>click to browse</strong></Typography>
              </Stack>
            </Box>
                ...
                <Stack direction="row" gap={1} flexWrap="wrap">
                  {attachments.map((f, i) => (
                    <Chip
                      ...
                      variant="soft"
                      endDecorator={
                        <IconButton size="sm" variant="plain" onClick={() => handleRemoveAttachment(i)}>
                          ✕
                        </IconButton>
                      }
                    >
          ...
          <DialogActions>
            <Button variant="plain" onClick={() => setOpenAttachModal(false)}>Close</Button>
