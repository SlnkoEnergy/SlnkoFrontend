    ...
    const fetchData = async () => {
      try {
        ...
        const response = await Axios.get(
          `/get-po-by-po_number?po_number=${po_Number}`,
          config
        );

        const pos = response?.data?.data?.[0];
        // console.log("Fetched PO data:", pos);

        if (!pos || typeof pos !== "object") {
          throw new Error("PO not found or invalid response format.");
        }
        ...
        setFormValues((prev) => ({
          ...
          vendor: pos.vendor || "",
          date: pos.date ? formatDate(pos.date) : "",
          item: pos.item || "",
          po_value: pos.po_value || "",
        }));
      } catch (err) {
