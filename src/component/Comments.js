    ...
    <Sheet variant="outlined" sx={{ borderRadius: "lg", p: 1.25, bgcolor: "background.surface" }}>
      ...
      >
        ...
        <Tooltip title="Italic (Ctrl/Cmd+I)" arrow>
          <IconButton variant="plain" color="neutral" onClick={() => focusAndApply("italic")} sx={toolBtn(states.italic)}>
            <FormatItalicIcon fontSize="small" />
          </IconButton>
        </Tooltip>
        <Tooltip title="Underline (Ctrl/Cmd+U)" arrow>
          <IconButton variant="plain" color="neutral" onClick={() => focusAndApply("underline")} sx={toolBtn(states.underline)}>
            <FormatUnderlinedIcon fontSize="small" />
          </IconButton>
        </Tooltip>
        <Tooltip title="Strikethrough" arrow>
          <IconButton variant="plain" color="neutral" onClick={() => focusAndApply("strikeThrough")} sx={toolBtn(states.strike)}>
        ...
        <Tooltip title="Bulleted list" arrow>
          <IconButton variant="plain" color="neutral" onClick={() => focusAndApply("insertUnorderedList")} sx={toolBtn(states.ul)}>
            <FormatListBulletedIcon fontSize="small" />
          </IconButton>
        </Tooltip>
        <Tooltip title="Numbered list" arrow>
          <IconButton variant="plain" color="neutral" onClick={() => focusAndApply("insertOrderedList")} sx={toolBtn(states.ol)}>
        ...
        <Dropdown open={showEmoji} onOpenChange={(_, open) => setShowEmoji(open)}>
          ...
          <Menu sx={{ p: 1, maxWidth: 320, maxHeight: 220, overflow: "auto" }}>
            <Box sx={{ display: "grid", gridTemplateColumns: "repeat(10, 1fr)", gap: 0.5, p: 0.5 }}>
              {emojis.map((e, i) => (
                <IconButton key={`${e}-${i}`} size="sm" variant="soft" onClick={() => insertEmoji(e)} sx={{ borderRadius: 8, fontSize: 18 }}>
                  {e}
                </IconButton>
              ))}
            </Box>
